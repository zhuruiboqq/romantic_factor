    ...
    <article class="">
        ...
        <div class="makeup_works_title">
            ...
            <div class="works_title">
            	<p>化妝師作品</p>
                <span>MAKEUP WORKS</span>
            </div>
            <div class="makeup_works_line right"></div>
        </div>
        
        <ul class="dress_content_list fix">
        	<c:forEach var="makeupWorks" items="${makeupWorksList}">
        		<%
        		i++;
        		if(i%4==1){ %>
        			<li class="dress_content_list_fri"><a href="JavaScript:;"><img src="${makeupWorks.img_url}" /></a><span>${makeupWorks.type}</span><%-- <span>${makeupWorks.day}</span> --%></li>
        		<%}else{ %>
        			 <li><a href="JavaScript:;"><img src="${makeupWorks.img_url}" /></a><span>${makeupWorks.type}</span><%-- <span>${makeupWorks.day}</span> --%></li>
        		<%} %>
        ...
        <div class="dress_btns">
        	...
        	<a class="fri_btn" href="makeupWorks.do?page=1&makeer_id=<%=request.getAttribute("id") %>">首頁</a>
            ...
            <c:choose>
                <c:when test="${pageid==page}">
           		 <a class="number one this_btn" href="makeupWorks.do?page=${page}&makeer_id=<%=request.getAttribute("id") %>">${page} </a>
            	</c:when>
                <c:otherwise>
            		<a class="number one" href="makeupWorks.do?page=${page}&makeer_id=<%=request.getAttribute("id") %>">${page} </a>
            	</c:otherwise>
            </c:choose>	
           	<c:if test="${page1 ne 0}">
           	 <c:choose>
                <c:when test="${pageid==page1}">
                	<a class="number two this_btn" href="makeupWorks.do?page=${page1}&makeer_id=<%=request.getAttribute("id") %>">${page1}</a>
                </c:when>
                <c:otherwise>
                	<a class="number two" href="makeupWorks.do?page=${page1}&makeer_id=<%=request.getAttribute("id") %>">${page1}</a>		
                </c:otherwise>
            </c:choose>
            ...
            <c:choose>
                <c:when test="${pageid==page2}">
                	 <a class="number three this_btn" href="makeupWorks.do?page=${page2}&makeer_id=<%=request.getAttribute("id") %>">${page2}</a>
                </c:when>
                <c:otherwise>
                	 <a class="number three" href="makeupWorks.do?page=${page2}&makeer_id=<%=request.getAttribute("id") %>">${page2}</a>	
                </c:otherwise>
            </c:choose>
            </c:if> 
            ...
            <a class="next_btn" href="makeupWorks.do?page=${nextPage}&makeer_id=<%=request.getAttribute("id") %>">下一頁</a>
            <a class="last_btn" href="makeupWorks.do?page=${pageCount}&makeer_id=<%=request.getAttribute("id") %>">尾頁</a> 
        </div>
